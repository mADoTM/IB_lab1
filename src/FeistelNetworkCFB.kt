class FeistelNetworkCFB(
    ...
    fun encrypt(): List<IntArray> {
        ...
        for (block in blocks64) {
            val encryptedBlock = encryptParts(previousBlock)
            val xorBlock = block xor encryptedBlock
            list.add(xorBlock)
            previousBlock = xorBlock
        }
    ...
    fun decrypt(blocks: List<IntArray>): String {
        ...
        var previousBlock = initVector
        for (block in blocks) {
            val decryptedBlock = encryptParts(previousBlock)
            val plainBlock = block xor decryptedBlock
            list.add(plainBlock)
            previousBlock = block
        }
